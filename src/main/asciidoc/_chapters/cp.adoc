...
** A wildcard path to be evaluated by link:https://commons.apache.org/proper/commons-io/javadocs/api-release/org/apache/commons/io/FilenameUtils.html[FilenameUtils.wildcardMatch]; path can specify scheme or not (e.g. `file:///usr/hbase/coprocessors` or for all filesystems `/usr/hbase/coprocessors`)
